...
export const CanvasEditor: React.FC<CanvasEditorProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* TOP PAGE NAVIGATION & ASPECT RATIO TOOLBAR */}
      <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 px-3 py-1.5 rounded-xl bg-zinc-900/95 border border-zinc-700/80 text-white shadow-2xl backdrop-blur-md text-xs select-none">
        {/* Document Title Badge */}
        {page && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-zinc-800 border border-zinc-700 text-gray-300 font-medium">
            <FileText size={13} className="text-sky-400" />
            <span className="max-w-[130px] truncate">{page.title}</span>
          </div>
        )}

        {/* Page Switcher Navigation Controls */}
        {notebookPages && notebookPages.length > 0 && (
          <div className="flex items-center gap-1 bg-zinc-800 p-0.5 rounded-lg border border-zinc-700">
            ...
            <select
              ...
              className="bg-transparent text-sky-400 font-bold px-1.5 py-0.5 outline-none cursor-pointer text-xs"
              title="Jump to Page"
            >
        ...
        {onAddPage && (
          <button
            onClick={onAddPage}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-sky-600 hover:bg-sky-500 font-semibold text-white shadow transition-all active:scale-95 cursor-pointer"
            title="Add Another Page to Same File"
          >
            <Plus size={15} />
            <span>Add Page</span>
          </button>
        ...
        {/* PAGE ASPECT RATIO SELECTOR */}
        <div className="flex items-center gap-1.5 bg-zinc-800 px-2 py-1 rounded-lg border border-zinc-700 text-gray-300">
          <RectangleHorizontal size={14} className="text-amber-400" />
          <select
            value={pageAspectRatio}
            onChange={(e) => onSetPageAspectRatio && onSetPageAspectRatio(e.target.value as PageAspectRatio)}
            className="bg-transparent font-medium text-amber-300 outline-none cursor-pointer text-xs"
            title="Change Page Size & Aspect Ratio"
          >
